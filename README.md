...
- **One occurrence that differs** -- A repeater describes an endless series and has nowhere to say that one of its occurrences is not like the rest. `Move One Occurrence` writes a `MOVED` line inside the entry -- `` `MOVED: [2026-08-20 Thu] -> <2026-08-27 Thu 18:00>` `` -- and `Cancel One Occurrence` adds the day to the series' own `EXDATE`; the series goes on repeating either way, and the agenda, the editor and the Google Calendar export all read what was written. Neither command asks for a date to be typed: the day is chosen from the ones the series actually falls on, with the days already cancelled or moved listed among them and marked as such. From the agenda both are reached through the ↻ (or ⚑) of the row itself. See [One occurrence that differs](#one-occurrence-that-differs).
...
- **An entry changed by saying what to change** -- `Edit Task from Phrase` takes "перенеси на пятницу в 16:00 и сделай срочной" over the entry the cursor stands in and moves the day, the hour and the priority in one write, where the commands for them are three separate steps. A phrase can name the keyword -- "отметь выполненной" -- and empty a field: "убрать дату", "без повтора". One write, one commit and one undo for the whole sentence; a word the rules do not know changes nothing at all and is named instead of half-applied. See [Changing an entry by saying what to change](#changing-an-entry-by-saying-what-to-change).
...
- **Git status of the source files** -- A chip in the agenda header counts the files of the current view by state -- unresolved conflicts (`!`), uncommitted changes (`●`), files touched by unpushed commits (`↑`), and files whose state could not be read at all (`?`) -- or says `✓ clean` when there is nothing to report. It expands to the list behind those numbers, grouped by the same states, and to the commits a push would send. Commit (only the view's changed files, never unrelated edits in the same repository), commit-and-sync, push and sync run from the same dropdown; a merge left unresolved takes the commit button away until it is settled in Source Control. Sync is the exchange in one press -- fetch, fast-forward a branch that is only behind, push one that is only ahead -- and it leaves a branch that has diverged exactly as it stands, naming it, because merging is a decision made in Source Control. Commit-and-sync is the two of them in that order, for the common case of writing a note here and reading it on the phone; a commit the user cancels or one that fails stops the round before the sync. Files reached through a symlink resolve to the repository behind them, including one outside the open workspace folders. Needs no setting and no configuration: the chip appears whenever the built-in Git extension is available -- a file outside git is one of the states it reports, not a reason to hide it. See [ADR-0016](docs/adr/0016-git-status-via-git-extension-api.md), [ADR-0020](docs/adr/0020-panel-does-not-resolve-conflicts.md) and [ADR-0022](docs/adr/0022-the-panel-syncs-but-never-merges.md).
- **Lines checked as they are written** -- The planning lines of an entry are read by the same rules the extractor applies, and a line it would refuse is warned about in the editor under the `markdown-org` source rather than silently dropped from the agenda: a `CREATED` stamp written active or in the wrong place, a `MOVED` line whose address carries an hour or a repeater, a move naming a day the series does not fall on, and a move standing in an entry that does not repeat at all. `Ctrl+.` on the warning offers the fix where there is one.
...
**Git status of the source files:** the chip on the right of the header counts the files behind the current view by state -- unresolved conflicts (`!`), uncommitted changes (`●`), files touched by commits the remote does not have (`↑`), and files whose state could not be read at all (`?`, outside git or in a repository VS Code declined to open). Expanding it names those files, groups them by the same states, and lists the commits a push would send. Four actions run from the same dropdown, all of them over the view's own files: **Commit** them without touching unrelated edits in the same repository, **Commit and sync** for the note written here and read on the phone, **Push** the branch, and **Sync** -- the exchange in one press: fetch, fast-forward a branch that is only behind, push one that is only ahead. A branch that has diverged is left exactly as it stands and named, because merging is a decision made in Source Control ([ADR-0022](docs/adr/0022-the-panel-syncs-but-never-merges.md)). Commit and sync is the two in that order, and a commit the user cancels or one git refuses stops the round before the sync. Git commits the whole index, so a repository holding changes staged elsewhere is named in a question before the commit is made; a merge left unresolved takes the commit button away until it is settled in Source Control.
...
The setting covers the agenda panel: what it renders, and what an action started from it says afterwards. The group menu on an overdue band, its undo, and the git chip's actions — Commit, Commit and sync, Push, Sync — raise their prompts, refusals, progress titles and status-bar confirmations in this language, and the numbers in them carry the digits of [`markdown-org.dateLocale`](#markdown-orgdatelocale), as the panel's own counts do.
